...
describe("The Markdown Transformer", () => {
  ...
  it("should not keep duplicate links", () => {
    const markdownText =
      "[Visible text link](url) Something else [Visible text link](url)";

    const transformedText = new MarkdownTransformer(markdownText).transform();

    expect(transformedText).toBe(
      "Visible text link [^anchor] Something else Visible text link [^anchor]\n\n[^anchor1]: url",
    );
  });
});
